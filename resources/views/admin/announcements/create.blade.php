<x-admin-layout title="Buat Pengumuman" header="Publikasikan pengumuman untuk member.">
    <x-slot name="actions">
    ...
    <form method="POST" action="{{ route('admin.announcements.store') }}">
        ...
        <x-card title="Detail Pengumuman" subtitle="Tulis konten pengumuman.">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <x-input name="title" label="Judul" required placeholder="Judul pengumuman" class="sm:col-span-2" />
                <div>
                ...
                <x-input name="published_at" label="Tanggal Publikasi" type="date" value="{{ old('published_at', now()->format('Y-m-d')) }}" />
            </div>
            <div class="mt-4">
                <x-label value="Isi Pengumuman" :required="true" />
                <textarea name="body" rows="5" placeholder="Tulis isi pengumuman..."
                          class="mt-1.5 block w-full rounded-xl border-0 bg-white px-3.5 py-2.5 text-sm text-slate-800 shadow-sm ring-1 ring-inset ring-slate-300 transition focus:ring-2 focus:ring-brand-500 dark:bg-slate-800 dark:text-slate-100 dark:ring-slate-700">{{ old('body') }}</textarea>
            </div>
        </x-card>
