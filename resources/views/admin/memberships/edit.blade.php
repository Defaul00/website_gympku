<x-admin-layout title="Edit Paket" header="Perbarui paket membership.">
    <x-slot name="actions">
    ...
    <form method="POST" action="{{ route('admin.memberships.update', $membership) }}">
        ...
        <x-card title="Detail Paket" subtitle="Informasi paket membership.">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <x-input name="name" label="Nama Paket" required value="{{ $membership->name }}" class="sm:col-span-2" />
                <x-input name="duration_months" label="Durasi (bulan)" type="number" min="1" max="60" required value="{{ $membership->duration_months }}" />
                <x-input name="price" label="Harga (Rp)" type="number" min="0" step="any" required value="{{ $membership->price }}" />
            </div>
            <div class="mt-4">
                <x-label value="Deskripsi" />
                <textarea name="description" rows="3"
                          class="mt-1.5 block w-full rounded-xl border-0 bg-white px-3.5 py-2.5 text-sm text-slate-800 shadow-sm ring-1 ring-inset ring-slate-300 transition focus:ring-2 focus:ring-brand-500 dark:bg-slate-800 dark:text-slate-100 dark:ring-slate-700">{{ old('description', $membership->description) }}</textarea>
            </div>
            <label class="mt-4 inline-flex cursor-pointer items-center gap-2.5">
                <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $membership->is_active))
