<x-admin-layout title="Edit Peralatan" header="Perbarui data peralatan gym.">
    <x-slot name="actions">
    ...
    <form method="POST" action="{{ route('admin.equipments.update', $equipment) }}">
        ...
        <x-card title="Detail Peralatan" subtitle="Informasi peralatan gym.">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <x-input name="name" label="Nama Peralatan" required value="{{ $equipment->name }}" />
                <x-input name="category" label="Kategori" required value="{{ $equipment->category }}" />
                <div>
                    ...
                    <select name="condition" required
                        ...
                        @foreach(['good' => 'Layak', 'needs_maintenance' => 'Perlu Service', 'poor' => 'Rusak'] as $key => $label)
                            <option value="{{ $key }}" @selected(old('condition', $equipment->condition) === $key)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <x-input name="last_maintenance" label="Maintenance Terakhir" type="date" value="{{ $equipment->last_maintenance?->format('Y-m-d') }}" />
                <x-input name="next_maintenance" label="Maintenance Berikutnya" type="date" value="{{ $equipment->next_maintenance?->format('Y-m-d') }}" />
            </div>
        </x-card>
